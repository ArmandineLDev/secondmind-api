-- Revert secondMind:stats_0005_rename_lead_views from pg
...
SELECT
  ...
  (SELECT COUNT(*) FROM opportunity op WHERE op.organization_id = o.id AND op.stage NOT IN ('won', 'lost'))              AS open_leads,
  (SELECT COALESCE(SUM(i.amount), 0) FROM invoice i
...
SELECT
  ...
  COUNT(*) FILTER (WHERE op.stage = 'won')                                                                 AS won_leads,
